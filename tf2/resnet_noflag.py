...
FLAGS_global_bn = True;
FLAGS_batch_norm_decay = 0.9;
FLAGS_sk_ratio = 0.;
FLAGS_se_ratio = 0.;
FLAGS_train_mode = 'pretrain';
FLAGS_fine_tune_after_block = -1;
# FLAGS = flags.FLAGS
  ...
  def __init__(self,
    ...
    if FLAGS_global_bn:
      ...
      self.bn = tf.keras.layers.experimental.SyncBatchNormalization(
          axis=axis,
          momentum=FLAGS_batch_norm_decay,
...
class ResidualBlock(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
    ...
    if use_projection:
      if FLAGS_sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
        if strides > 1:
    ...
    if FLAGS_se_ratio > 0:
      self.se_layer = SE_Layer(filters, FLAGS_se_ratio, data_format=data_format)
...
class BottleneckBlock(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    if FLAGS_sk_ratio > 0:
      self.conv_relu_dropblock_layers.append(
          SK_Conv2D(filters, strides, FLAGS_sk_ratio, data_format=data_format))
    else:
      self.conv_relu_dropblock_layers.append(
          Conv2dFixedPadding(
              filters=filters,
    ...
    if FLAGS_se_ratio > 0:
      self.conv_relu_dropblock_layers.append(
          SE_Layer(filters, FLAGS_se_ratio, data_format=data_format))
...
class Resnet(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
    ...
    trainable = (
        FLAGS_train_mode != 'finetune' or FLAGS_fine_tune_after_block == -1)
    ...
    if FLAGS_train_mode == 'finetune' and FLAGS_fine_tune_after_block == 0:
      trainable = True
    ...
    if FLAGS_train_mode == 'finetune' and FLAGS_fine_tune_after_block == 1:
      trainable = True
    ...
    if FLAGS_train_mode == 'finetune' and FLAGS_fine_tune_after_block == 2:
      trainable = True
    ...
    if FLAGS_train_mode == 'finetune' and FLAGS_fine_tune_after_block == 3:
      trainable = True
    ...
    if FLAGS_train_mode == 'finetune' and FLAGS_fine_tune_after_block == 4:
  ...
  def call(self, inputs, training):
    ...
    for i, layer in enumerate(self.block_groups):
      if FLAGS_train_mode == 'finetune' and FLAGS_fine_tune_after_block == i:
